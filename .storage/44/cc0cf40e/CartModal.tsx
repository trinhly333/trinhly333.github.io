...
const CartModal = () => {
  ...
  useEffect(() => {
    ...
    const applicableCodes = discountCodes.filter(code => 
      code.is_active && subtotal >= code.min_order_amount
    );
  ...
  const getNextBetterDiscount = () => {
    ...
    const betterCodes = discountCodes.filter(code => {
      if (!code.is_active || subtotal >= code.min_order_amount) return false;
      ...
      if (code.discount_type === 'fixed') {
        ...
        if (code.max_discount_amount) {
          potentialDiscount = Math.min(potentialDiscount, code.max_discount_amount);
        }
      }
            ...
            {items.length === 0 ? (
              ...
              <div className="space-y-4">
                ...
                {remainingForFreeShipping > 0 && (
                  <div className="bg-secondary-accent-blue bg-opacity-10 border border-secondary-accent-blue border-opacity-20 rounded-lg p-4">
